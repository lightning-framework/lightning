...
public class Injector {
   ...
   */
  public Object[] getInjectedArguments(Method m, Object ...initialArgs) throws Exception {
    try {
      return getInjectedArguments(m.getParameters(), initialArgs);
   ...
   * @throws Exception On failure (for example, if unable to resolve all parameters).
   */
  public Object[] getInjectedArguments(Constructor<?> m, Object ...initialArgs) throws Exception {
    try {
      return getInjectedArguments(m.getParameters(), initialArgs);
    } catch (InjectionException e) {
      throw new InjectionException("Failed to inject dependencies for method " + m + ": " + e.getMessage());
   ...
   * @throws Exception On failure (for example, if unable to resolve all parameters).
   */
  public Object[] getInjectedArguments(Parameter[] parameters, Object ...initialArgs) throws Exception {
    if (initialArgs.length > parameters.length) {
      throw new InjectionException("Provided initial arguments exceed parameter count.");
    }
