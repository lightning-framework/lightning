 ...
 *      dbp.forEachRowInPage(j, (row) -> {
 *        doSomethingWith(row);
 *      });
 *   }
 *
 *   try (NamedPreparedStatement q = dbp.getQueryForPage(j)) {
 *     try (ResultSet r = q.exeuteQuery()) {
 *        while (r.next()) {
 *          doStuffWith(r);
 *        }
 *     }
...
public class DatabasePaginator {
  ...
  /**
   * NOTE: Caller must close() to avoid leaking resources.
   * @param pageNumber
   * @return A statement to execute to fetch the rows for pageNumber.
   * @throws Exception
   */
  public NamedPreparedStatement getQueryForPage(long pageNumber) throws Exception {
    if (!hasPage(pageNumber)) {
      throw new NotFoundException(String.format("Page %d does not exist.", pageNumber));
    }

    String pageQuery = (query.endsWith(";") ? query.substring(0, query.length() - 1) : query);
    pageQuery += String.format(" LIMIT %d, %d;", pageNumber * pageSize, pageSize);
    logger.debug("Fetching page=" + pageNumber + " query=" + pageQuery);

    return db.prepare(pageQuery, parameters);
  }
   ...
   */
  public void forEachRowInPage(long pageNumber, DatabasePaginatorVisitor fn) throws Exception {
    try (NamedPreparedStatement statement = getQueryForPage(pageNumber);
         ResultSet result = statement.executeQuery()) {
      while (result.next()) {
        fn.apply(result);
      }
